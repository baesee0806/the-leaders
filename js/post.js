...
export const getPostContent = async (event) => {
console.log("성공");
const 쿼리스트링 = new URLSearchParams(window.location.search)
//     console.log(쿼리스트링.get())

const response = {
  "2DN8dm003RqugrU0eVhV" : {

  }
}
 const docRef = doc(dbService, 'post', 쿼리스트링.get('uid'));
 const docSnap = await getDoc(docRef);
 if (docSnap.exists()) {
console.log(docSnap.data());
 
  const detail = ` <div class="product">
    <div class="thumbnail" style="background-image: url('${docSnap.data().postUrl}')"></div>
    <div class="flex-grow-1 p-4">
      <h5 class="title"><a href="post?id=${docSnap.id}">${docSnap.data().title}</a></h5>
      <p class="date">${docSnap.data().date}</p>
      <p class="price">${docSnap.data().score}점</p>
